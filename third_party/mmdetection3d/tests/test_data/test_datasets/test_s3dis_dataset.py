...
def test_seg_getitem():
    ...
    pipelines = [
        ...
        dict(
            type="PointSegClassMapping", valid_cat_ids=tuple(range(len(class_names))), max_cat_id=13
        ),
        dict(
...
def test_multi_areas():
    ...
    assert np.all(
        s3dis_dataset.scene_idxs == np.array([0, 0, 1, 2, 2, 3, 4, 5, 6, 6, 7, 8, 9, 9, 10, 10, 10])
    )
