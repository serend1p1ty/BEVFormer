...
def test_point_shuffle():
    ...
    input_dict = dict(
        points=points.clone(), pts_instance_mask=ins_mask.copy(), pts_semantic_mask=sem_mask.copy()
    )
...
def test_object_range_filter():
    ...
    expected_repr_str = (
        "ObjectRangeFilter(point_cloud_range=" "[0.0, -40.0, -3.0, 70.4000015258789, 40.0, 1.0])"
    )
    assert repr_str == expected_repr_str
...
def test_points_sample():
    ...
    expected_repr_str = (
        f"PointSample(num_points={num_points}, " f"sample_range={sample_range}, " "replace=False)"
    )
    assert repr_str == expected_repr_str
